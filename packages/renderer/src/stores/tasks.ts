...
window.events?.receive('task-removed', (task: unknown) => {
  removeTask((task as TaskInfo).id);
});

export function isStatefulTask(task: TaskInfo): task is StatefulTask {
  return 'state' in task;
}

export function isNotificationTask(task: TaskInfo): task is NotificationTask {
