...
// Recursively extract all gotos from the navigation registry
let goToItems: GoToInfo[] = $derived.by(() => {
  function extract(entry: NavigationRegistryEntry): Array<GoToInfo> {
    return (entry.items ?? []).reduce(
      (accumulator, current) => {
        accumulator.push(...extract(current));
        return accumulator;
      },
      entry.gotos ?? ([] as Array<GoToInfo>),
    );
  }
