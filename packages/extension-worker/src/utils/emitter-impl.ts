 ...
 */
export type Event<T> = (listener: (e: T) => unknown, thisArgs?: unknown, disposables?: DisposableGroup) => Disposable;
...
class CallbackList implements Iterable<Callback> {
  ...
  public add(callback: Function, context: unknown = undefined, bucket?: Disposable[]): void {
    if (!this._callbacks) {
      this._callbacks = [];
      this._contexts = [];
    }
...
export class Emitter<T = unknown> {
  ...
  canPushDisposable(candidate?: DisposableGroup): candidate is { push(disposable: Disposable): void } {
    return Boolean(candidate && (candidate as { push(): void }).push);
  }
  canAddDisposable(candidate?: DisposableGroup): candidate is { add(disposable: Disposable): void } {
